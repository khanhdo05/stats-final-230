...
ui <- fluidPage(
  ...
  titlePanel("Find Similar Colleges"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      #numericInput(inputID = 'n', label = 'Number of Clusters (Specificity of Similarity)', value = 100),
#says this is an unused argument, but would like this to be defined for the number of centers
...
server <- function(input, output) {
  ...
  filtered_data <- eventReactive(input$show_result, {
    ...
    if(input$enrollment_filter == 0){
      df <- select(df, !UGDS)
    }
    
    if(input$ACT_filter == 0){
      df <- select(df, !ACT_MEDIAN)
    }
    
    if(input$admission_filter == 0){
      df <- select(df, !ADM_RATE)
    }
    
    if(input$debt_filter == 0){
      df <- select(df, !GRAD_DEBT_MDN)
    }
...
#swap centers with n (defined above)
