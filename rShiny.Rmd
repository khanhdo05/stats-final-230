...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton("show_result", "Show Similar Colleges")
    ),
...
server <- function(input, output) {
  ...
  output$cluster_plot <- renderPlotly({
    ...
    if ("INSTNM" %in% colnames(df)) {
      school_names <- df$INSTNM
      features <- select(df, -INSTNM)
    } else {
      school_names <- paste("School", 1:nrow(df))
      features <- df
    }
    ...
    if ("1" %in% input$show_only_cluster) {
      ...
      selected_cluster <- pca_data$Cluster[pca_data$School == selected_school]
