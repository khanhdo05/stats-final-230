...
feature3data <- data %>%
  ...
  select(UGDS, ADM_RATE, ACT_MEDIAN, GRAD_DEBT_MDN) %>% #can't find median earnings, add back INSTNM after scaling.
  mutate(GRAD_DEBT_MDN = as.numeric(GRAD_DEBT_MDN)) %>% # for some reason this was a character
  drop_na()
...
# Sketch visualization
feature3data <- scale(feature3data)
k2 <- kmeans(feature3data, centers = 2, nstart = 25) #number of centers can be changed for "how similar" user wants
fviz_cluster(k2, data = feature3data)
```
